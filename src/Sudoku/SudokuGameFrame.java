...
public class SudokuGameFrame extends JInternalFrame {
	...
	private Color goodBlueColor = new Color(7, 87, 91);
	private Color badRedColor = new Color(237, 67, 55);
	...
	private void SetupForm() {
		setBackground(new Color(196, 223, 230));
		setClosable(true);
		setMaximizable(true);
		setResizable(false);
		...
		//allows user to reset grid if lots of mistakes are made
		...
		//allows the user to see the solution on one click
				...
				sgc.addKeyListener(new KeyAdapter() {
					public void keyTyped(KeyEvent e) {
						...
						if ( ((c < '1') || (c > '9')) || sgc.getIsLocked()) {
							...
							//if cheat hints are on, color it red it 
							...
							if (_allowHintColoring) {
								if (!ValidateCellAgainstSolution(sgc)) {
									...
									sgc.setForeground(goodBlueColor);
								}
							}
	 ...
	 * methods against solution (hints if the cell is not a valid value)
	 */
	...
	private void CreateNewGame() {
		...
		_gameGrid = new SudokuMainGrid(_difficultyLevel);
		...
		for(Component g : grids) {
			...
			for (Component b : boxes) {
				SudokuGridCell sgc = (SudokuGridCell)b;
				sgc.setText("");
				sgc.setBackground(new Color(196, 223, 230));
				...
				if (sgvc.getValue() > 0) {
					...
					sgc.setForeground(new Color(0, 59, 70));
					sgc.setBackground(new Color(102, 165, 173));
	...
	private void ResetGameGrid() {
		...
		/**
		 *Tricky part - Put the values in every single
	...
	class RestoreGameAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				_gameGrid = (SudokuMainGrid)ois.readObject();
				...
				for(Component g : grids) {
					...
					for (Component b : boxes) {
						...
						if (gameGridVal > 0) {
							...
							if (startGridVal > 0) {
								//if was original start value, color it
								sgc.setForeground(new Color(0, 59, 70));
								sgc.setBackground(new Color(102, 165, 173));
								sgc.setIsLocked(true);
							}
